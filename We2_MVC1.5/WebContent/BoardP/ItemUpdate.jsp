...
<!--  게시판 영역 - css에서 #contents 블록의 테두리선(border)를 없애주시면 됩니다. -->			
			<form action="ShareUpdate" method="post" enctype="multipart/form-data" name="frm">
					...
					<td>
						<c:choose>
							<c:when test="${empty BoardUpdate.itemPath }">
								...
								<img src="/We2/BoardP/pic/noimg.JPG">
							</c:when>
							<c:otherwise>
									<!-- 기본설정경로 : 서블릿경로
										E:\JavaSmartWeb\mywork_web\.metadata\.plugins\org.eclipse.wst.server.core\tmp2\wtpwebapps -->
								<img src="/We2/BoardP/pic/${BoardUpdate.itemPath }">
							</c:otherwise>
						</c:choose>
					</td>
				</tr>
				...
				<input type="button" value="취소" onclick="location.href='ShareList?page=${page}'">
			</form>
<!--  게시판 영역 끝 -->
</body>
</html>