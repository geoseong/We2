...
<!--  게시판 영역 - css에서 #contents 블록의 테두리선(border)를 없애주시면 됩니다. -->			
			<form action="ItemUpdate" method="get" name="frm" >
					...
					<td>
						<table>
							...
							<tr>
								<th>사   진 </th>
								<td colspan="3">
								<a href="/We2/BoardC/pic/${BoardContent.itemPath }">${BoardContent.itemPath }</a>
					...
					<td>
						<c:choose>
							<c:when test="${empty BoardContent.itemPath }">
								...
								<img src="/We2/BoardC/pic/noimg.JPG" style="width: 300px; ">
							</c:when>
							<c:otherwise>
									<!-- 기본설정경로 : 서블릿경로
										E:\JavaSmartWeb\mywork_web\.metadata\.plugins\org.eclipse.wst.server.core\tmp2\wtpwebapps -->
								<img src="/We2/BoardC/pic/${BoardContent.itemPath }" style="width: 300px; ">
							</c:otherwise>
						</c:choose>
					</td>
				</tr>
				...
				<input type="button" value="목록" onclick="location.href='ItemList?page=${page}'">
				<input type="button" value="제거하기" onclick="location.href='ItemDelete?itemNum=${BoardContent.itemNum }&page=${page}'">
		</form>
<!--  게시판 영역 끝 -->
</body>
</html>