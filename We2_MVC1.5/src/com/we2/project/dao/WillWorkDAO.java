...
public class WillWorkDAO {
	...
	public List<WillWorkVO> selectpjtusers(String pjtcode){
		//최근 등록한 상품 먼저 출력하기
	...
	// 사용자 머릿수계산
	public int Pjtcount(String pjtcode){
		...
		try{
			...
			System.out.println("DAO - 머릿수 : " + counts);
		}catch(Exception e){
			e.printStackTrace();
		}finally{
			DBManager.close(conn, pstmt, rs);
	...
	// 사용자 리스트를 배열로 받기
	public String[] Pjtlists(String pjtcode){
		...
		try{
			...
			while(rs.next()){
				...
				System.out.println("DAO - user"+i+" -- " + users[i]);
				i++;
			} //end while
